...
public class UserController extends BaseController {
    ...
    public Object allInformation(@RequestHeader("Authorization") String token){
        User user = getUser();
        User userFind = userService.findOne(user.getCuraNumber());
        userFind.setPassword(null);
        Map<String, Object> map = new HashMap<>();
        map.put("user", userFind);
        List<Commend> commends = commendService.getCommends(user.getCuraNumber());
    ...
    public Object login(@RequestBody Map<String, Object> body) {
        if (body.containsKey("curaNumber") && body.containsKey("password")) {
            int curaNumber = (Integer) body.get("curaNumber");
            String password = (String) body.get("password");
            User userFind = userService.findOne(curaNumber);
            if (userFind == null){
                return jsonResult.failed(400, "账号不存在", HttpStatus.BAD_REQUEST);
            }
            User user = userService.checkUser(curaNumber, password);
            if (user == null){
    ...
    public Object getUsersByInformation(@RequestHeader("Authorization") String token, @RequestParam("information") String information){
        ...
        if(users == null){
            return jsonResult.failed(400, "查询不到", HttpStatus.BAD_REQUEST);
        }
    ...
    public Object getOne(@RequestHeader("Authorization") String token, @RequestParam("curaNumber") int curaNumber){
        ...
        userMap.put("curaNumber", user.getCuraNumber());
        ...
        Friend friend = friendService.queryByCuraNumber(curaNumber, mainUser.getCuraNumber());
        if (friend!=null){
            isFriend = true;
        }
        ...
        List<Commend> commends = commendService.getCommends(mainUser.getCuraNumber());
        map.put("commends", commends);
        return jsonResult.success(map);
    }
    ...
    public Object updateHeadUrl(@RequestHeader("Authorization") String token, UserFriend userFriend){
        ...
        try{
            ...
            fileConverse.logout();
            User modifyUser = userService.modifyHead(user.getCuraNumber(), url);
    ...
    public Object addGroup(@RequestHeader("Authorization") String token, @RequestBody Map<String, Object> body){
        if(body.containsKey("groupName")){
            User user = getUser();
            Group group = groupService.addGroup(user.getCuraNumber(), (String)body.get("groupName"));
    ...
    public Object removeGroup(@RequestHeader("Authorization") String token, @RequestBody Map<String, Object> body){
        ...
        if(body.containsKey("groupId")) {
            String result = groupService.removeGroup(user.getCuraNumber(), (int) body.get("groupId"));
            if(result == null){
    ...
    public Object modifyGroupName(@RequestHeader("Authorization") String token, @RequestBody Map<String, Object> body){
        ...
        if(body.containsKey("groupId")&&body.containsKey("groupName")){
            ...
            if (groupService.getDefaultGroup(user.getCuraNumber()).getGroupId() == groupId){
    ...
    public Object getUserAllGroups(@RequestHeader("Authorization") String token){
        User user = getUser();
        return jsonResult.success(groupService.getUserGroups(user.getCuraNumber()));
    }
